...
// Agora estamos em ChatBot/scripts/video => para chegar em /TarefaN3 use 3 níveis
$baseDir = dirname(__DIR__, 3); // .../TarefaN3
...
// DEBUG TEMPORÁRIO
if ($titulo === '' || (!$hasFile && $link === '') || ($hasFile && $link !== '')) {
...
$stmt = $conn->prepare("
  INSERT INTO TB_TREINAMENTOS_BOT_REFORMA (titulo, origem, link, status, data_inicio)
  VALUES (?, ?, ?, 'PROCESSANDO', ?)
");
if (!$stmt) {
  http_response_code(500);
...
if (!file_exists($script)) {
  ...
  $stmt = $conn->prepare("UPDATE TB_TREINAMENTOS_BOT_REFORMA SET status='ERRO', data_fim=NOW(), log=? WHERE id=?");
  if ($stmt) {
...
// Converte caminho absoluto (FS) em URL pública, sem hardcode de /TarefaN3
function fs_to_url(string $absPath): ?string {
  $doc = realpath($_SERVER['DOCUMENT_ROOT'] ?? '');
  $abs = realpath($absPath);
  if (!$doc || !$abs) return null;

  $doc = rtrim(str_replace('\\','/', $doc), '/');
  $abs = str_replace('\\','/', $abs);

  if (strpos($abs, $doc) !== 0) return null; // fora do docroot
...
if ($arquivoFs && file_exists($arquivoFs)) {
  $arquivoUrl = fs_to_url($arquivoFs);
}

// ===== Atualiza histórico =====
$status = ($ret === 0) ? 'CONCLUIDO' : 'ERRO';
$stmt = $conn->prepare("
  UPDATE TB_TREINAMENTOS_BOT_REFORMA
     SET status = ?,
         arquivo_json = ?,
         data_fim = NOW(),
         log = ?
