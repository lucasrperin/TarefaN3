...
$res = $conn->query("SELECT MAX(data_geracao) AS ultima FROM TB_EMBEDDINGS_REFORMA WHERE tipo = 'artigos'");
$row = $res->fetch_assoc();
$ultima = $row['ultima'] ? date('d/m/Y H:i:s', strtotime($row['ultima'])) : 'Nunca';

// busca última de VÍDEO
$resvideo = $conn->query("SELECT MAX(data_geracao) AS ultima FROM TB_EMBEDDINGS_REFORMA WHERE tipo = 'video'");
...
$resweb = $conn->query("SELECT MAX(data_geracao) AS ultima FROM TB_EMBEDDINGS_REFORMA WHERE tipo = 'website'");
...
$hist = $conn->query("
  SELECT id, titulo, origem, link, arquivo_json, status, data_inicio, data_fim
    FROM TB_TREINAMENTOS_BOT_REFORMA
ORDER BY id DESC
   LIMIT 50
");
$historico = [];
...
if (isset($_SERVER['HTTP_X_PARTIAL']) && $_SERVER['HTTP_X_PARTIAL'] === 'log') {
  ...
  $q = $conn->prepare("SELECT log FROM TB_TREINAMENTOS_BOT_REFORMA WHERE id=?");
  $q->bind_param('i', $id);
  $q->execute();
  $res = $q->get_result();
  $log = '';
...
$execEtapasFs       = realpath(__DIR__ . '/../../chatbotreforma/webchat/executar_etapas.php');
$chamarProcVidFs    = realpath(__DIR__ . '/../../chatbotreforma/scripts/video/chamar_processa_video.php');
$chamarProcSiteFs   = realpath(__DIR__ . '/../../chatbotreforma/scripts/website/chamar_processa_site.php');
$listarJsonSiteFs   = realpath(__DIR__ . '/../../chatbotreforma/scripts/website/listar_json_site.php');
$excluirJsonSiteFs  = realpath(__DIR__ . '/../../chatbotreforma/scripts/website/excluir_json_site.php');
...
<head>
  ...
  <title>Configuração do Agente - Reforma Tributária</title>
...
<div class="d-flex-wrapper">
  <?php include __DIR__ . '/../../components/sidebar_bot_reforma.php'; ?>

  <div class="w-100 flex-grow-1 d-flex flex-column">
    <!-- ===== Header (mantido exatamente) ===== -->
    <div class="header d-flex justify-content-between align-items-center p-3 border-bottom">
      <h3 class="mb-0"><i class="bi bi-gear me-2"></i>Configuração do Agente - Reforma Tributária</h3>
...
<script src="../../chatbotreforma/webchat/js/config_bot.js?v=<?= @filemtime(__DIR__ . '/../../chatbotreforma/webchat/js/config_bot.js') ?: time() ?>" defer></script>
<script>
