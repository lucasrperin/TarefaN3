-- Arquivo corrigido para import no MySQL
-- Ajustes: adicionado IF NOT EXISTS no schema, ponto-e-vírgula faltantes, DEFAULTs corrigidos,
-- vírgulas faltando em constraints e tipos ajustados (DATETIME/CURRENT_TIMESTAMP onde apropriado).

CREATE SCHEMA IF NOT EXISTS TarefaN3;
USE TarefaN3;
...
CREATE TABLE IF NOT EXISTS TB_ANALISES (
 Id INT PRIMARY KEY AUTO_INCREMENT,
...
CREATE TABLE IF NOT EXISTS TB_SISTEMA_CONVER (
...
CREATE TABLE IF NOT EXISTS TB_STATUS_CONVER (
    id INT AUTO_INCREMENT PRIMARY KEY,
...
CREATE TABLE IF NOT EXISTS TB_ANALISTA_CONVER (
    id INT AUTO_INCREMENT PRIMARY KEY,
    nome VARCHAR(255) NOT NULL UNIQUE
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4;
...
CREATE TABLE IF NOT EXISTS TB_CONVERSOES (
    id INT AUTO_INCREMENT PRIMARY KEY,
...
CREATE TABLE IF NOT EXISTS TB_TREINAMENTOS (
    id INT AUTO_INCREMENT PRIMARY KEY,
    data DATE NOT NULL,
    hora TIME NOT NULL,
...
CREATE TABLE IF NOT EXISTS TB_RECORRENTES_CARDS (
  ...
  CONSTRAINT fk_recorrentes_cards FOREIGN KEY (recorrente_id) REFERENCES TB_RECORRENTES(id) ON DELETE CASCADE
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4;
...
CREATE TABLE IF NOT EXISTS TB_OKR (
    ...
    criado_em DATETIME DEFAULT CURRENT_TIMESTAMP,
    atualizado_em DATETIME DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
    CONSTRAINT fk_okr_equipe FOREIGN KEY (idEquipe) REFERENCES TB_EQUIPE(id),
...
CREATE TABLE IF NOT EXISTS TB_META (
    ...
    CONSTRAINT fk_meta_okr FOREIGN KEY (idOkr) REFERENCES TB_OKR(id) ON DELETE CASCADE,
    UNIQUE KEY uq_okr_ano_desc (idOkr, ano, descricao),
    INDEX idx_meta_okr (idOkr)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4;

/* ========= TB_OKR_ATINGIMENTO ========= */
CREATE TABLE IF NOT EXISTS TB_OKR_ATINGIMENTO (
    id INT AUTO_INCREMENT PRIMARY KEY,
    idMeta INT NOT NULL,
    ano YEAR NOT NULL,
    mes TINYINT NOT NULL,
    realizado DECIMAL(8,4) NULL,
    realizado_seg INT NULL,
    criado_em DATETIME DEFAULT CURRENT_TIMESTAMP,
    CONSTRAINT fk_ating_meta FOREIGN KEY (idMeta) REFERENCES TB_META(id) ON DELETE CASCADE,
    UNIQUE KEY uq_meta_mes (idMeta, ano, mes),
    INDEX idx_ating_meta (idMeta)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4;


CREATE TABLE IF NOT EXISTS TB_OKR_NIVEL (
    idOkr INT NOT NULL,
...
CREATE TABLE IF NOT EXISTS TB_ANALISES_PROD (
 ...
 chkParado CHAR(1),
 criado_em DATETIME DEFAULT CURRENT_TIMESTAMP,
 ult_edicao DATETIME COMMENT 'Data da ultima edição'
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4;

CREATE TABLE IF NOT EXISTS tb_avaliacoes_chatbot (
    id INT AUTO_INCREMENT PRIMARY KEY,
    user_id INT NOT NULL,
    nota INT NOT NULL,
