...
// Para preencher os selects do filtro
...
// Definir datas do mês atual se não vier filtro do usuário
if (empty($_GET['data_inicio']) && empty($_GET['data_fim'])) {
    $primeiro_dia = date('Y-m-01');
    $ultimo_dia = date('Y-m-t');
    $_GET['data_inicio'] = $primeiro_dia;
    $_GET['data_fim'] = $ultimo_dia;
}

// Query base TB_ANALISES
$sql = "SELECT
            tas.Id as Codigo,
...
// Filtros dinâmicos
if (!empty($_GET['data_inicio'])) {
    $sqlProd .= " AND DATE(tas.criado_em) >= '" . $_GET['data_inicio'] . "'";
}
if (!empty($_GET['data_fim'])) {
    $sqlProd .= " AND DATE(tas.criado_em) <= '" . $_GET['data_fim'] . "'";
}
if (!empty($_GET['situacao'])) {
    $sqlProd .= " AND tas.idSituacao = '" . $_GET['situacao'] . "'";
}
if (!empty($_GET['sistema'])) {
    $sqlProd .= " AND tas.idSistema = '" . $_GET['sistema'] . "'";
}
if (!empty($_GET['status'])) {
    $sqlProd .= " AND tas.idStatus = '" . $_GET['status'] . "'";
}
if (!empty($_GET['parceiro'])) {
    $sqlProd .= " AND tas.idParceiro = '" . $_GET['parceiro'] . "'";
}
$sqlProd .= " ORDER BY tas.Id DESC";
...
// Junta tudo em um único array para totalizadores e gráficos
$allRows = array_merge($rows, $rowsProd);

// Inicializa totalizadores e arrays
$totalFichas = 0;
$totalAnaliseN3 = 0;
$totalAuxilio = 0;
$totalParado = 0;
$fichasPorSistema = [];
$analisesPorSistemaN3 = [];
$analisesPorSistemaProd = [];
...
foreach ($allRows as $row) {
    ...
    if ($situacao === "Ficha Criada") {
        ...
        // Análises N3 e Produto separados
        if ($situacao === "Analise N3") {
            $totalAnaliseN3++;
            $analisesPorSistemaN3[$sis] = ($analisesPorSistemaN3[$sis] ?? 0) + 1;
        } elseif ($situacao === "Analise Setor Produto") {
            $totalAnaliseN3++; // se quiser somar tudo junto no geral
            $analisesPorSistemaProd[$sis] = ($analisesPorSistemaProd[$sis] ?? 0) + 1;
        }
    }

    if ($situacao === "Auxilio Suporte/Vendas") {
        $totalAuxilio++;
    }
    if ($parado === "S") {
        $totalParado++;
    ...
    // Gráficos por mês (só se tiver data)
    if (!empty($row['Hora_ini'])) {
...
// ====== Cálculo da média de horas usando só Analise N3 da TB_ANALISES ======
$analiseN3ComHora = 0;
$totalHoras = 0;
foreach ($rows as $row) {
    if (trim($row['Situacao']) === "Analise N3" && !empty($row["Total_hora"])) {
        $analiseN3ComHora++;
...
if ($analiseN3ComHora > 0) {
    $mediaSegundos = round($totalHoras / $analiseN3ComHora);
...
$systems = array_unique(array_merge(
    ...
    array_keys($analisesPorSistemaN3),
    array_keys($analisesPorSistemaProd)
));
...
<body class="bg-light">
    <div class="d-flex-wrapper">
        <?php include __DIR__ . '/components/sidebar_index.php'; ?>
            ...
            <div class="content container-fluid">                   
                            ...
                            <div class="card-header d-flex align-items-center modern-card-header">
                                ...
                                <div class="d-flex gap-2 align-items-center">
                                    ...
                                    <?php if ($cargo === 'Produto'): ?>
                                        <button type="button" class="btn btn-outline-primary btn-sm" data-bs-toggle="modal" data-bs-target="#filterModalProduto">
                                            <i class="fa-solid fa-filter"></i>
                                        </button>
                                        <?php include 'Public/Modals/filter_modal_produto.php'; ?>
                                    <?php else: ?>
                                ...
                                <div class="table-responsive access-scroll">
                                    ...
                                    if ($cargo === 'Produto') {
                                        ...
                                        if (isset($rows) && count($rows) > 0) {
                                            foreach($rows as $row) {
                                                echo "<tr>";
                                                echo "<td class='sobrepor'>" . htmlspecialchars($row["Descricao"]) . "</td>";
                                                ...
                                                echo "<td>" . htmlspecialchars($row["Total_hora"]) . "</td>";
                                                if ($cargo === 'Admin') {
                ...
                <div class="tab-pane fade" id="pane-relatorios" role="tabpanel" aria-labelledby="tab-relatorios">
                    ...
                    <div class="systems-scroll mb-4">
                        ...
                        ?>
                        <div class="row row-cols-2 row-cols-sm-3 row-cols-md-4 row-cols-lg-3 g-4 mb-4">
                            <?php foreach ($systems as $sis): 
                                $cntFicha      = $fichasPorSistema[$sis]         ?? 0;
                                $cntAnalise  = $analisesPorSistemaN3[$sis]     ?? 0;
                                $cntAnaliseProd= $analisesPorSistemaProd[$sis]   ?? 0;
                                $totalAnalises = $cntAnalise + $cntAnaliseProd;

                                // Percentual de fichas
                                $pctFicha = $totalAnalises
                                    ? round($cntFicha / $totalAnalises * 100)
                                    : 0;

                                $lowerSis = strtolower($sis);
                                $logoPath = $logosNormalized[$lowerSis] ?? "/Public/Image/Concorrente.png";
                            ?>
                                <div class="col">
                                    <div class="layout8-card h-100">
                                        ...
                                        <span class="layout8-badge badge-analise-prod">
                                            <i class="fa-solid fa-chart-line"></i>
                                            <?= $cntAnaliseProd ?> Análises Produto
                                        </span>
                                        </div>
                        ...
                        <div id="filterPeriod" style="display: none;">
                            <div class="row">
                                <div class="col-md-6 mb-3">
                                    ...
                                    <input type="date" class="form-control" id="data_inicio" name="data_inicio" value="<?php echo isset($_GET['data_inicio']) ? $_GET['data_inicio'] : $primeiro_dia; ?>">

                                </div>
                                <div class="col-md-6 mb-3">
                                    <label for="data_fim" class="form-label">Data Fim:</label>
                                    <input type="date" class="form-control" id="data_fim" name="data_fim" value="<?php echo isset($_GET['data_fim']) ? $_GET['data_fim'] : $ultimo_dia; ?>">
                                </div>
                            </div>
                        </div>
    ...
    <script>
        document.addEventListener("DOMContentLoaded", function () {
            // IDs das tabelas que terão destaque de status
            ["tabelaAnalises", "tabelaAnalisesProd"].forEach(function(tableId) {
                const tabela = document.getElementById(tableId);
                if (!tabela) return;

                // Descobre em que coluna está o "Status" nesta tabela
                let statusIndex = -1;
                tabela.querySelectorAll("thead tr").forEach(tr => {
                    tr.querySelectorAll("th").forEach((th, i) => {
                        if (th.textContent.trim().toLowerCase() === "status") statusIndex = i;
                    });
                });
                if (statusIndex === -1) return;

                // Aplica a cor na célula correta de cada linha
                tabela.querySelectorAll("tbody tr").forEach(row => {
                    let statusCell = row.cells[statusIndex];
  ...
  <!-- Script para pesquisar na tabela -->
    ...
    $(document).ready(function(){
        $("#searchInput").on("keyup", function() {
            ...
            $("#tabelaAnalisesProd tbody tr").filter(function() {
            $(this).toggle($(this).text().toLowerCase().indexOf(value) > -1);
            });
        });
        });
